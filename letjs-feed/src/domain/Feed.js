...
const formatters = {
  link(source, model, formatter) {
    if (!source.link) {
      return 'javascript:alert("你点击了我，这儿链接是空的")'
    }
    return source.link
  },
  poster(source, model, formatter) {
    return source.poster ? source.poster : '发帖人'
  },

  imgSrc(source, model, formatter) {
    let num = Math.round(Math.random() * imgData.length)
    let url = imgData[num]
    let start = 0
    while (imgNoList.includes(url) && start < 5) {
      start++
      num = Math.round(Math.random() * imgData.length)
      url = imgData[num]
    }
    imgNoList.push(num)
    return url
...
export class Feed extends Model {
  constructor(source, customFormatters) {
    ...
    this.define({
      feedId: 111,
      feedName: 'feedName',
      imgSrc: 'http://a.hiphotos.baidu.com/image/pic/item/e824b899a9014c087eb617650e7b02087af4f464.jpg',
      title: '这里是标题这里是标题',
      link: '',
      desc: '这里是描述这里是描述这 里是描述这里是描述这 里是描述这里是描述这里是描述这里是描述这里是描述这里是描述这里是描述这里是 描述这里是描述这里是描述这里是描述这里是描述这里是描述这里是描述 这里是描述这里是描述这里是描述',
      poster: '发帖人',
      posterId: 123456,
